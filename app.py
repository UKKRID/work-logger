...
# Local Fallback
DB_PATH = os.path.join(os.path.dirname(__file__), 'worklog.db')
...
def supabase_insert(table, data):
    ...
    return resp.json()[0] if resp.ok and resp.json() else None
...
if __name__ == '__main__':
    if not USE_SUPABASE:
        print("⚠️  Running in LOCAL mode (Supabase not configured)")
        print("   Configure .env file to enable cloud sync")
        init_db()
    else:
        print("☁️  Running in SUPABASE mode (Cloud sync enabled)")
